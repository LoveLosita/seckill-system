...
func (s *UserServiceImpl) UserRegister(ctx context.Context, req *user.UserRegisterRequest) (resp *user.UserRegisterResponse, err error) {
	...
	if result == true { //已经存在
		return &user.UserRegisterResponse{Status: &user_resp.UsernameExists}, nil
	}
	//2.检查用户参数是否合法
	//2.1.检查性别是否合法
	if !(req.Gender == "male" || req.Gender == "female") {
		return &user.UserRegisterResponse{Status: &user_resp.WrongGender}, nil
	}
	//2.2.检查邮箱是否唯一
	...
	if result == true {
		return &user.UserRegisterResponse{Status: &user_resp.EmailExists}, nil
	}
	//2.3.检查手机号是否唯一
	...
	if result == true {
		return &user.UserRegisterResponse{Status: &user_resp.PhoneNumberExists}, nil
	}
	//3.插入新用户信息
	//3.1.加密密码
	hashedPwd, err := utils.HashPassword(req.Password)
	if err != nil {
		retErr := user_resp.InternalErr(err)
	...
	return &user.UserRegisterResponse{Status: &user_resp.Ok}, nil
}
...
func (s *UserServiceImpl) UserLogin(ctx context.Context, req *user.UserLoginRequest) (resp *user.UserLoginResponse, err error) {
	...
	if result == false {
		return &user.UserLoginResponse{Status: &user_resp.WrongUsrName}, nil
	}
	//2.接下来检验密码
	//2.1.获取数据库中存储的密码
	...
	if result == false {
		return &user.UserLoginResponse{Status: &user_resp.WrongPassword}, nil
	}
	//3.登录成功，返回token
	//3.1.先获取用户id
	...
	//3.3.返回结果
	return &user.UserLoginResponse{Status: &user_resp.Ok, Data: &respData}, nil
}
...
func (s *UserServiceImpl) TokenRefresh(ctx context.Context, req *user.TokenRefreshRequest) (resp *user.TokenRefreshResponse, err error) {
	...
	if token == nil || status != emptyStatus {
		return &user.TokenRefreshResponse{Status: &user_resp.InvalidToken}, nil
	}
	//2.生成新的token
	...
	respData.AccessToken = accessToken
	respData.RefreshToken = refreshToken
	return &user.TokenRefreshResponse{Status: &user_resp.Ok, Data: &respData}, nil
}
